...
.nav{display:grid;gap:6px;margin-top:24px}.nav a{color:#dbe5f5;padding:12px;border-radius:9px;font-weight:700}.nav a:hover{background:#ffffff12;text-decoration:none}
...
<a href="{{url_for('dashboard')}}"><span>Overview</span></a><a href="{{url_for('products_page')}}"><span>Products</span></a><a href="{{url_for('orders_page')}}"><span>Orders</span></a><a href="{{url_for('customers_page')}}"><span>Customers</span></a><a href="{{url_for('events_page')}}"><span>Event History</span></a><a href="{{url_for('reports_page')}}"><span>Daily Reports</span></a>{% if cloudwatch_url %}<a href="{{cloudwatch_url}}" target="_blank" rel="noopener"><span>Monitoring</span></a>{% endif %}</nav></aside>
...
<div class="toolbar"><form class="search" method="get" action="{{url_for('search')}}"><input name="q" placeholder="Search products, orders, or customers"><button>Search</button></form><a class="btn alt" href="{{url_for('reports_page')}}">Reports</a></div>
<div class="cards">{%for label,key in [('Products','products'),('Customers','customers'),('Orders','orders'),('Low stock','low'),('Failed orders','failed')]%}<div class="card"><div class="label">{{label}}</div><div class="number">{{summary[key]}}</div></div>{%endfor%}</div>
...
<div class="panel"><div class="heading"><h3>Latest S3 report</h3><a href="{{url_for('reports_page')}}">All reports</a></div>{%if reports%}<a href="{{reports[0].url}}" target="_blank">{{reports[0].key}}</a><p class="muted">Generated {{shown(reports[0].last_modified)}} · {{reports[0].size}} bytes</p>{%else%}<div class="empty">No generated reports found. Verify the daily report Lambda, schedule, and S3 parameter.</div>{%endif%}</div>
"""
...
def products_page():
    ...
    rows = all_rows("""SELECT p.product_id,p.name,COALESCE(c.name,'Uncategorized') category,p.price,p.stock_quantity,p.reorder_threshold FROM products p LEFT JOIN categories c ON p.category_id=c.category_id WHERE p.deleted_at IS NULL AND (%s='' OR p.name LIKE %s OR CAST(p.product_id AS CHAR) LIKE %s) ORDER BY p.product_id""",(q,like,like))
    ...
    return page("Products", LIST, q=q, placeholder="Search product name or ID", heading="Products & inventory", rows=rows, table=table)
...
def orders_page():
    ...
    if g: return g
    q = request.args.get("q", "").strip(); like = f"%{q}%"
    rows = all_rows("""SELECT order_id,customer_id,status,order_date,total_amount FROM orders WHERE %s='' OR CAST(order_id AS CHAR) LIKE %s OR CAST(customer_id AS CHAR) LIKE %s OR status LIKE %s ORDER BY order_date DESC LIMIT 200""",(q,like,like,like))
    ...
    return page("Orders", LIST, q=q, placeholder="Search order ID, customer ID, or status", heading="Orders", rows=rows, table=table)
